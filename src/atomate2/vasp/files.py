...
def copy_non_vasp_outputs(
    ...
    # # find required files
    # files = ("cluster_space.cs", "force_constants.fcs", "parameters.txt", 
    #          "fitting_data.json", "structure_data.json", "phonopy_params.yaml",
    #          "thermal_data.json")
    # required_files = [get_zfile(directory_listing, r) for r in files]
    ...
    for file in ["cluster_space.cs", "force_constants.fcs", "parameters.txt", 
             "fitting_data.json", "structure_data.json", "phonopy_params.yaml",
             "thermal_data.json", "perturbed_structures.json", "perturbed_forces_new.json",
             "structure_data.json"]:
        found_file = get_zfile(directory_listing, file, allow_missing=True)
        if found_file is not None:
            optional_files.append(found_file)
